...
riot.tag2('grid2', '<div class="gridwrap" riot-style="height:{opts.height}px"> <div class="gridbody" riot-style="left:{fixedLeftWidth}px;top:{rowHeight}px;bottom:0px"> <div class="fixedLeft" riot-style="transform:translate3d({0-overlay.scrollLeft}px,{0-overlay.scrollTop}px,0px);backface-visibility: hidden;width:{fixedLeftWidth}px;bottom:1px;z-index:2"> <gridcelltag class="cell {active:cell.active}" tag="{cell.tag}" value="{cell.text}" cell="{cell}" each="{cell in visCells.main}" onclick="{parent.handleClick}" no-reorder riot-style="position: absolute;left:{cell.left}px;top:{cell.top}px;width:{cell.width}px;height:{rowHeight}px">{cell.text}</gridcelltag> </div> </div> <div class="gridbody" ref="header" riot-style="height:{rowHeight}px;margin-right:15px"> <div class="header" riot-style="top:0px;left:0px;width:{scrollWidth}px;height:{rowHeight}px"> <div class="headercell" each="{headers.main}" no-reorder riot-style="transform:translate3d({left}px,0px,0px); backface-visibility: hidden;width:{width}px;height:{rowHeight}px">{text}</div> </div> </div> <div class="gridbody" riot-style="width:{fixedLeftWidth}px;height:{opts.height-2}px"> <div class="fixedLeft" riot-style="transform:translate3d(0px,{0-overlay.scrollTop}px,0px);backface-visibility: hidden;width:{fixedLeftWidth}px;bottom:1px;z-index:2"> <div class="header" riot-style="top:{overlay.scrollTop}px;left:0px;width:{fixedLeftWidth}px;height:{rowHeight}px"> <div class="headercell" each="{headers.fixed}" riot-style="top:0px;left:{left}px;width:{width}px;height:{rowHeight}px">{text}</div> </div> <gridcelltag class="cell {active:cell.active}" tag="{cell.tag}" value="{cell.text}" cell="{cell}" each="{cell in visCells.fixed}" onclick="{parent.handleClick}" no-reorder riot-style="position: absolute;left:{cell.left}px;top:{cell.top}px;width:{cell.width}px;height:{rowHeight}px">{cell.text}</gridcelltag> </div> </div> <div class="gridbody overlay" ref="overlay" onscroll="{scrolling}" riot-style="overflow:auto;left:0px;top:{rowHeight}px;bottom:0px"> <div class="scrollArea" riot-style="background:rgba(0,0,0,0.005);width:{scrollWidth}px;height:{scrollHeight-rowHeight}px"></div> </div> </div>', 'grid2 { display: block; -webkit-font-smoothing: antialiased; text-rendering: optimizeSpeed; } grid2 .scrollArea { transform: translateZ(0); } grid2 .gridwrap { position: relative; display: block; border: 1px solid #ccc; font-family: sans-serif; font-size: 14px; } grid2 .gridbody { position: absolute; overflow: hidden; top: 0; left: 0; right: 0; bottom: 0; transform: translateZ(0); backface-visibility: hidden; } grid2 .fixedLeft { position: absolute; top: 0; bottom: 0; } grid2 .cell, grid2 .headercell { position: absolute; box-sizing: border-box; padding: 10px 5px; whitespace: no-wrap; overflow: hidden; background: #fff; border: 1px solid #eee; border-width: 0 1px 1px 0; cursor: pointer; } grid2 .cell.active { background: #eee; } grid2 .header { position: absolute; z-index: 1; overflow: hidden; transform: translateZ(0); }', '', function(opts) {
...
this.on('mount', (function(_this) {
  return function() {
    ...
    _this.activeCells = [];
    _this.activeRows = [];
    _this.rowHeight = +opts.rowheight || 40;
    _this.gridbody = _this.root.querySelectorAll(".gridbody");
    _this.overlay = _this.refs.overlay;
    _this.overlay.addEventListener('click', _this.pushThroughClick);
    return _this.overlay.addEventListener('dlbclick', _this.pushThroughClick);
  };
})(this));

this.on('unmount', (function(_this) {
  return function() {
    _this.overlay.removeEventListener('click', _this.pushThroughClick);
    return _this.overlay.removeEventListener('dlbclick', _this.pushThroughClick);
  };
})(this));

this.on('update', (function(_this) {
  return function() {
    if (!_this.gridbody || !opts.data || !opts.columns) {
      return;
    }
    if (opts.columns && opts.data && (_this.columns !== opts.columns || _this.data !== opts.data)) {
      _this.data = opts.data;
      _this.columns = opts.columns;
      _this.rows = [];
      calcPos();
      _this.visCells = calcVisible(_this.rows, _this.overlay, _this.rowHeight);
    }
    if (!_this.visCells) {
      return _this.visCells = calcVisible(_this.rows, _this.overlay, _this.rowHeight);
    } else {
      return _this.visCells = reCalc(_this.visCells, _this.rows, _this.overlay, _this.rowHeight);
    }
  };
})(this));
...
this.scrolling = (function(_this) {
  return function(e) {
    ...
    _this.refs.header.scrollLeft = _this.overlay.scrollLeft;
    return _this.update();
  };
})(this);
